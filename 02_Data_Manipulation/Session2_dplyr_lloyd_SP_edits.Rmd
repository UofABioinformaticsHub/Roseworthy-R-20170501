...
toothData <- read.csv("toothData.csv")

#Check for missing data
tooth.na <- is.na(toothData$len)
sum(tooth.na)

#Alternative way to subset
tooth.na <- is.na(toothData[,1])
sum(tooth.na)

#how to select just the columns 'len' and 'dose'?
tooth.len.dose <- toothData[,c(1,3)]

#how to filter for a particular row? say tooth length more than mean tooth length?
tooth.len.abv.mean <- toothData[toothData$len > mean(toothData$len),]
```
# dplyr
```{r, echo = TRUE, fig.width=3, fig.height = 3, fig.align='center'}
library(dplyr)
